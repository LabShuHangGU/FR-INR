...
# Replace this file in the lib of the DVGO: https://github.com/sunset1995/DirectVoxGO.
# Set the iteration number of fine train 25000 to reproduce the paper results.

class Fourier_reparam_linear(nn.Module):
    def __init__(self,in_features,out_features,high_freq_num=64,low_freq_num=64,phi_num=64,alpha=0.05):
    ...
    def __init__(self, xyz_min, xyz_max,
        ...
        if rgbnet_dim <= 0:
            ...
            self.rgbnet = nn.Sequential(
                nn.Linear(dim0, rgbnet_width), nn.ReLU(inplace=True),
                *[
                    nn.Sequential(Fourier_reparam_linear(rgbnet_width, rgbnet_width), nn.ReLU(inplace=True))
                    for _ in range(rgbnet_depth-2)
                ],
                nn.Linear(rgbnet_width, 3),
            )
