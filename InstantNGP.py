...
# We reproduc the InstantNGP by https://github.com/kwea123/ngp_pl.
# Replace the 'network.py' by this file. And set the epoch number 110.

class param_layer(nn.Module):
    def __init__(self,in_features,out_features,high_freq_num,low_freq_num,phi_num,amplitude,omega_0,bases_type):
...
class Fourier_reparam_linear(nn.Module):
    def __init__(self,in_features,out_features,high_freq_num=32,low_freq_num=32,phi_num=64,alpha=0.01):
...
# high_freq_num=low_freq_num=32 ,phi_num=64, amplitude=0.01
class param_MLP(nn.Module):
    def __init__(self,n_input_dims, n_output_dims,hidden_features,hidden_layers,high_freq_num,low_freq_num,phi_num,alpha):
        super(param_MLP, self).__init__()
        ...
        self.alpha=alpha
        layers=[]
        layers.append(nn.Linear(n_input_dims,hidden_features))
        layers.append(nn.ReLU())
        for _ in range(hidden_layers-1):
            layers.append(Fourier_reparam_linear(hidden_features,hidden_features,high_freq_num,low_freq_num,phi_num,amplitude,omega_0,bases_type))
            layers.append(nn.ReLU())
        linear_layer = nn.Linear(hidden_features, n_output_dims)
        nn.init.xavier_uniform_(linear_layer.weight)  # Xavier 
...
class MLP(nn.Module):
    def __init__(self, n_input_dims, n_output_dims, activation, output_activation, n_neurons, n_hidden_layers):
        super(MLP, self).__init__()

        # 定义网络结构
        layers = []
        layers.append(nn.Linear(n_input_dims, n_neurons))
        layers.append(nn.ReLU())  # 隐藏层激活函数
...
mlp=MLP(32,3,'Sigmoid','Sigmoid',64,2)
mlp.cuda()

param_mlp=param_MLP(32,3,64,2,32,32,64,0.01)
param_mlp.cuda()
...
class NGP(nn.Module):
    def __init__(self, scale, rgb_act='Sigmoid'):
        ...
        mlp_small=MLP(encoding.n_output_dims,16,'ReLU','None',64,1)
        mlp_small.cuda()
        network=mlp_small
        ...
        print('Here')
        # self.rgb_net = mlp
        self.rgb_net = param_mlp
        print(self.rgb_net)
        print(self.xyz_encoder)
    ...
    def density(self, x, return_feat=False):
        ...
        h = self.xyz_encoder(x)#x是32位的，但是经过self.xyzencoder之后，变成了16位，为了在32位下计算，重新转为32位
